...
export class ApplicationRepository {
    ...
    async findAll(filterApplicationDto: FilterApplicationDto) {
     const { page = 1, limit = 5, status, search } = filterApplicationDto;
     ...
     const skip = PaginationUtil.getSkip(page, limit);
    
      {
          ...
          if(filterApplicationDto.status) {
              match.status = filterApplicationDto.status;
          }
